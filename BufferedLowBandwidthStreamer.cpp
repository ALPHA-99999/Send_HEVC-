...
BufferedLowBandwidthStreamer::BufferedLowBandwidthStreamer(
...
bool BufferedLowBandwidthStreamer::initialize()
{
    ...
    const int packetPayloadBytes =
#if LOW_BANDWIDTH_TRANSPORT_MODE == LOW_BANDWIDTH_TRANSPORT_SERIAL_FIXED300
        kFixedSerialPayloadSize - kHeaderSize;
#else
        std::max(1, m_transportConfig.payloadSize);
#endif
    const int maxSendHz = std::max(1, m_transportConfig.maxSendHz);
    m_maxBacklogBytes = static_cast<size_t>(packetPayloadBytes * maxSendHz);
    m_initialized = true;
    return true;
}

bool BufferedLowBandwidthStreamer::initCamera()
{
...
bool BufferedLowBandwidthStreamer::initSocket()
{
...
bool BufferedLowBandwidthStreamer::initSerial()
{
#ifdef _WIN32
...
bool BufferedLowBandwidthStreamer::initEncoder()
{
...
void BufferedLowBandwidthStreamer::startStreaming()
{
    ...
    m_captureThread = std::thread(&BufferedLowBandwidthStreamer::captureLoop, this);
    m_processThread = std::thread(&BufferedLowBandwidthStreamer::processLoop, this);
    m_sendThread = std::thread(&BufferedLowBandwidthStreamer::sendLoop, this);
}

void BufferedLowBandwidthStreamer::stopStreaming()
{
...
void BufferedLowBandwidthStreamer::captureLoop()
{
    ...
    while (!m_stopRequested.load()) {
        ...
        if (m_frameQueue.size() >= m_maxFrameQueueSize) {
...
void BufferedLowBandwidthStreamer::processLoop()
{
    while (true) {
        QueuedFrame queued;
        {
        ...
        {
            std::lock_guard<std::mutex> sendLock(m_sendQueueMutex);
            if (m_backlogBytes > m_maxBacklogBytes) {
        ...
        if (m_captureConfig.showPreview) {
            cv::imshow("Buffered LowBandwidth Preview", processed);
            if (cv::waitKey(1) == 27) {
                m_stopRequested = true;
                m_isStreaming = false;
                m_frameQueueCv.notify_all();
...
void BufferedLowBandwidthStreamer::sendLoop()
{
    using clock = std::chrono::steady_clock;
    clock::time_point nextAllowedSendTime = clock::now();
    clock::time_point lastPacketSentTime = clock::time_point{};
    const int maxSendHz = std::max(1, m_transportConfig.maxSendHz);
    const auto minPacketInterval = std::chrono::microseconds(1000000 / maxSendHz);

    while (true) {
        {
            std::unique_lock<std::mutex> lock(m_sendQueueMutex);
            m_sendQueueCv.wait(lock, [this] {
                return m_stopRequested.load() || !m_sendQueue.empty();
            });
        ...
        if (now < nextAllowedSendTime) {
            ...
            while (true) {
                ...
                if (current >= nextAllowedSendTime) {
                    break;
                }

                const auto remaining = nextAllowedSendTime - current;
                if (remaining > coarseSleepThreshold) {
                    std::this_thread::sleep_until(nextAllowedSendTime - std::chrono::milliseconds(1));
                } else {
        ...
        if (!sendOnePacket(m_sendQueue.front())) {
            m_stopRequested = true;
            m_isStreaming = false;
            m_frameQueueCv.notify_all();
            m_sendQueueCv.notify_all();
        ...
        const auto sentTime = clock::now();
        if (lastPacketSentTime != clock::time_point{}) {
            const auto intervalMs = std::chrono::duration_cast<std::chrono::milliseconds>(
                sentTime - lastPacketSentTime).count();
            std::cout << "[Send] packet interval " << intervalMs << " ms" << std::endl;
        }
        lastPacketSentTime = sentTime;
        nextAllowedSendTime = sentTime + minPacketInterval;
...
cv::Rect BufferedLowBandwidthStreamer::computeEffectiveRoi(const cv::Size& frameSize) const
{
...
cv::Mat BufferedLowBandwidthStreamer::preprocessFrame(const cv::Mat& frame)
{
...
cv::Mat BufferedLowBandwidthStreamer::buildMotionMask(const cv::Mat& grayFrame, double& motionRatio)
{
...
void BufferedLowBandwidthStreamer::applyStaticSuppression(const cv::Mat& source,
...
void BufferedLowBandwidthStreamer::applyCenterProtection(const cv::Mat& source, cv::Mat& output) const
{
...
void BufferedLowBandwidthStreamer::applyTrail(const cv::Mat& grayFrame,
                                              const cv::Mat& motionMask,
                                              double motionRatio,
                                              cv::Mat& output)
{
...
bool BufferedLowBandwidthStreamer::encodeFrame(const cv::Mat& frame, uint32_t frameIndex)
{
    ...
    sws_scale(m_swsContext, srcSlice, srcStride, 0, frame.rows, m_yuvFrame->data, m_yuvFrame->linesize);
    ...
    while (ret >= 0) {
        ...
        enqueueEncodedFrame(m_packet->data, m_packet->size, frameIndex);
...
bool BufferedLowBandwidthStreamer::flushEncoder()
{
...
void BufferedLowBandwidthStreamer::enqueueEncodedFrame(const uint8_t* data, int size, uint32_t frameIndex)
{
    if (!data || size <= 0) {
        return;
    }
    ...
    m_backlogBytes += frame.data.size();
    m_sendQueue.push_back(std::move(frame));
    m_sendQueueCv.notify_one();
}

bool BufferedLowBandwidthStreamer::sendOnePacket(EncodedFrame& frame)
{
#if LOW_BANDWIDTH_TRANSPORT_MODE == LOW_BANDWIDTH_TRANSPORT_SERIAL_FIXED300
    constexpr int payloadSize = kFixedSerialPayloadSize - kHeaderSize;
#else
    const int payloadSize = std::max(1, m_transportConfig.payloadSize);
#endif

    if (frame.nextOffset >= frame.data.size()) {
        m_backlogBytes -= frame.data.size();
    ...
    const int chunkSize = std::min<int>(payloadSize, static_cast<int>(frame.data.size() - frame.nextOffset));

#if LOW_BANDWIDTH_TRANSPORT_MODE == LOW_BANDWIDTH_TRANSPORT_SERIAL_FIXED300
    std::vector<uint8_t> innerPacket(kFixedSerialPayloadSize, 0);
    innerPacket[0] = static_cast<uint8_t>((frame.frameIndex >> 8) & 0xFF);
    innerPacket[1] = static_cast<uint8_t>(frame.frameIndex & 0xFF);
    innerPacket[2] = static_cast<uint8_t>((frame.nextPacketIndex >> 8) & 0xFF);
    innerPacket[3] = static_cast<uint8_t>(frame.nextPacketIndex & 0xFF);
    innerPacket[4] = static_cast<uint8_t>((frame.data.size() >> 24) & 0xFF);
    innerPacket[5] = static_cast<uint8_t>((frame.data.size() >> 16) & 0xFF);
    innerPacket[6] = static_cast<uint8_t>((frame.data.size() >> 8) & 0xFF);
    innerPacket[7] = static_cast<uint8_t>(frame.data.size() & 0xFF);
    memcpy(innerPacket.data() + kHeaderSize, frame.data.data() + frame.nextOffset, chunkSize);
    ...
    m_totalBytesSent.fetch_add(serialFrame.size());
#else
    std::vector<uint8_t> packet(kHeaderSize + chunkSize);
    packet[0] = static_cast<uint8_t>((frame.frameIndex >> 8) & 0xFF);
    packet[1] = static_cast<uint8_t>(frame.frameIndex & 0xFF);
    packet[2] = static_cast<uint8_t>((frame.nextPacketIndex >> 8) & 0xFF);
    packet[3] = static_cast<uint8_t>(frame.nextPacketIndex & 0xFF);
    packet[4] = static_cast<uint8_t>((frame.data.size() >> 24) & 0xFF);
    packet[5] = static_cast<uint8_t>((frame.data.size() >> 16) & 0xFF);
    packet[6] = static_cast<uint8_t>((frame.data.size() >> 8) & 0xFF);
    packet[7] = static_cast<uint8_t>(frame.data.size() & 0xFF);
    memcpy(packet.data() + kHeaderSize, frame.data.data() + frame.nextOffset, chunkSize);
    ...
    frame.nextOffset += chunkSize;
    ++frame.nextPacketIndex;
...
void BufferedLowBandwidthStreamer::logAvError(const std::string& prefix, int errnum) const
{
...
void BufferedLowBandwidthStreamer::cleanup()
{
    if (m_swsContext) {
